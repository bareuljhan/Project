...
void Effect::SetLeftRight(bool direction)
{
	_sprite->GetLeftRight() = direction;
}
...
void Effect::CreateAction(wstring file, Vector2 maxFrame, Vector2 size, float speed, Action::Type type)
{
	...
	srvPath = L"Resource/Texture/CupHead/Effect/" + srvPath + L".png";

	MyXML xml = MyXML(xmlPath, srvPath);
	
	_action = make_shared<Action>(xml.GetClips(), name, type, speed);
