...
Vector2 RectCollider::GetCloserVertex(shared_ptr<RectCollider> rect1, shared_ptr<RectCollider> rect2)
{
    Vector2 otherRectPos = rect2->GetTransform()->GetWorldPos();
    Vector2 leftTop = Vector2(rect1->Left(), rect1->Top());
    Vector2 rightTop = Vector2(rect1->Right(), rect1->Top());
    Vector2 leftBottom = Vector2(rect1->Left(), rect1->Bottom());
    Vector2 rightBottom = Vector2(rect1->Right(), rect1->Bottom());

    int leftTopL = (int)(leftTop - otherRectPos).Length();
    int rightTopL = (int)(rightTop - otherRectPos).Length();
    int leftBottomL = (int)(leftBottom - otherRectPos).Length();
    int rightBottomL = (int)(rightBottom - otherRectPos).Length();

    int min = min(leftTopL, min(rightTopL, min(leftBottomL, rightBottomL)));
